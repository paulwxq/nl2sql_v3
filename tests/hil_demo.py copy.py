    ...
    try:
        ...
        while "__interrupt__" in result:
            ...
            if interrupt_type == "review_sql":
                ...
                if ans == "yes":
                    result = app.invoke(Command[tuple[()]](resume={"approved": True}), config)
                else:
                    result = app.invoke(Command[tuple[()]](resume={"approved": False}), config)
                ...
                if ans == "yes":
                    result = app.invoke(Command[tuple[()]](resume={"confirmed": True}), config)
                else:
                    result = app.invoke(Command(resume={"confirmed": False}), config)
        ...
        final_result = result.get("result", "已完成")
